...
3. При передачи по eBGP добавляется AS в AS-PATH. 
   При передачи между участниками iBGP AS в AS-PATH не добавляется
...
6. По разному работает loop prevention:
   ...
   - для iBGP - правило split-horizon, которое не дает рассылать анонсы, полученные по iBGP соседям.
   - Для RR: 
