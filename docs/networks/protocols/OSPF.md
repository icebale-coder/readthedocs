...
Возможна только на маршрутизаторах раница области, таких как ABR и ASBR, соответственно это фильтрация LSA 3 и LSA 5.
...
```bash
	...
	ip prefix-list FILTER-INTO-AREA-34 seq 10 permit 0.0.0.0/24 le 32
	!
	router ospf 1
		area 32 filter-list preafix FILTER-INTO-AREA-34 in
```
...
Она просто не добавляет указанный маршрут в таблицу маршрутизации, но в базе OSPF маршрут до сети будет.
...
Пример настройки на оборудовании cisco
```bash
	ip prefix-list FILTER-10.16.0.1 seq 5 deny 10.16.1.0/24
	ip prefix-list FILTER-10.16.0.1 seq 10 permit 0.0.0.0/24 le 32
	!
	router ospf 1
		distribute-list prefix FILTER-10.16.1.0 in
```
